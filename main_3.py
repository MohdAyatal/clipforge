...
async def download_video(job_id: str, url: str) -> str:
    ...
    # Check if deno is available (it is on Render by default)
    deno_result = subprocess.run(["which", "deno"], capture_output=True, text=True)
    deno_path = deno_result.stdout.strip()
    ...
    if deno_path:
        # Use deno as JS runtime (available by default on Render)
        cmd += ["--js-runtimes", f"deno:{deno_path}"]
    else:
        # Fallback: use android/mweb clients (no JS runtime needed)
    ...
    if proc.returncode != 0:
        raise Exception(f"Download failed: {err.decode()[:400]}")
